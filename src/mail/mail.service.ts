...
export class MailService {
  ...
  // ── Public method called by CheckoutService ────────────────────────────
  ...
  async sendOrderConfirmation(order: Order): Promise<void> {
    ...
    try {
      ...
      // Never crash the request over email — just log
  ...
  private buildTicketHtml(order: Order): string {
...
<body style="margin:0;padding:0;background:#f5f5f5;font-family:Arial,sans-serif;">
  ...
  private buildTicketText(order: Order): string {
    const event     = order.event;
    const eventName = event?.name ?? 'Event';
    const total     = Number(order.total_amount).toFixed(2);
